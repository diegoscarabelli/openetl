...
class TestGarminProcessor:
    ...
    def test_process_strength_metrics(self, processor, mock_session) -> None:
        """
        Test _process_strength_metrics field mapping and pop behavior.
    ...
    def test_process_strength_metrics_skips_missing_pk_fields(
        self, processor, mock_session
    ) -> None:
        """
        Test _process_strength_metrics skips exercises missing category or name.
    ...
    def test_process_strength_metrics_empty_sets(self, processor, mock_session) -> None:
        """
        Test _process_strength_metrics with no summarized exercise sets.
    ...
    @patch("dags.pipelines.garmin.process.upsert_model_instances")
    def test_process_strength_routing(
        self, mock_upsert, processor, mock_session
    ) -> None:
        """
        Test that strength_training activities route to _process_strength_metrics.
        ...
        activity_data = {
            ...
            "activityType": {"typeId": 71, "typeKey": "strength_training"},
        ...
        # Mock upsert to return a persisted activity with activity_id.
        mock_activity = MagicMock()
        ...
        with patch.object(processor, "_process_strength_metrics") as mock_strength:
            processor._process_single_activity(
                copy.deepcopy(activity_data), mock_session
            )

        # Assert.
        mock_strength.assert_called_once()

    def test_process_exercise_sets(self, processor, mock_session, temp_dir) -> None:
        """
        Test _process_exercise_sets field mapping and ML exercise selection.
        ...
        file_path = temp_dir / "123_EXERCISE_SETS_22320029355_2025-03-27.json"
        with open(file_path, "w") as f:
            json.dump(exercise_sets_data, f)
    ...
    def test_process_exercise_sets_skips_null_message_index(
        self, processor, mock_session, temp_dir
    ) -> None:
        """
        Test _process_exercise_sets skips sets with null messageIndex.
        ...
        # Assert - only 1 record (null messageIndex skipped).
        mock_session.add_all.assert_called_once()
        records = mock_session.add_all.call_args[0][0]
        assert len(records) == 1
        assert records[0].set_idx == 0

    def test_process_exercise_sets_empty(
        self, processor, mock_session, temp_dir
    ) -> None:
        """
        Test _process_exercise_sets with empty exercise sets.
