...
- **Garmin extract no longer issues backwards-range API calls when the pipeline is already up to date** (ported from `garmin-health-data` [#77](https://github.com/diegoscarabelli/garmin-health-data/issues/77)): the resolved start date (`prev_data_interval_end_success`, or an inverted caller-supplied range) can land after the resolved end date. Previously `extract()` still discovered accounts, constructed a `GarminExtractor`, and issued per-data-type API calls over the backwards range (including `MENSTRUAL_CYCLE_DAY`'s ~90-day retroactive-refresh window) before finally raising `AirflowSkipException` ("No Garmin Connect data found"). It now raises `AirflowSkipException` immediately when the resolved start is after the end, before any account discovery or API calls. Same-day windows stay inclusive and still dispatch.
