...
This document describes the ETL data pipeline which processes data sourced from Garmin Connect. The pipeline collects comprehensive health, fitness, and activity data using a vendored [`garmin_client/`](garmin_client/) module that handles SSO authentication and exposes the 16 Garmin Connect API endpoints used by the pipeline. The vendored client uses a five-strategy login fallback chain (portal web with `curl_cffi` TLS fingerprint impersonation, plain `requests` portal web, mobile cffi, mobile requests, and SSO embed widget as a last-resort) with a 30-45s anti-rate-limit delay between SSO page GET and credential POST, native DI OAuth2 token exchange and rotating-refresh persistence, and multi-factor authentication (MFA) support. No external Garmin client library is required.
...
|-------------|--------|-------------|---------------|
| **Sleep** | Sleep stage duration, movement, levels, restless moments, heart rate (redundant with Heart Rate dataset), stress levels (redundant with Stress dataset), body battery (redundant with Stress dataset), HRV (5 mins interval time series), breathing disruptions count (event time-series), detailed scores, and sleep need. | `/wellness-service/wellness/dailySleepData/{display_name}?date={date}&nonSleepBufferMinutes=60` | [12345678_SLEEP_2025-08-07T12:00:00Z.json](./example_data/12345678_SLEEP_2025-08-07T12:00:00Z.json) |
| **Stress** | Stress level and body battery measurements (3 mins interval time-series). | `/wellness-service/wellness/dailyStress/{date}` | [12345678_STRESS_2025-08-07T12:00:00Z.json](./example_data/12345678_STRESS_2025-08-07T12:00:00Z.json) |
| **Respiration** | Breathing rate readings (2 mins interval and 1 hour aggregates time-series) and aggregated statistics. | `/wellness-service/wellness/daily/respiration/{date}` | [12345678_RESPIRATION_2025-08-07T12:00:00Z.json](./example_data/12345678_RESPIRATION_2025-08-07T12:00:00Z.json) |
| **Heart Rate (HR)** | Heart rate readings (2 mins interval time-series). | `/wellness-service/wellness/dailyHeartRate/{display_name}?date={date}` | [12345678_HEART_RATE_2025-08-07T12:00:00Z.json](./example_data/12345678_HEART_RATE_2025-08-07T12:00:00Z.json) |
| **Training Readiness** | Daily training readiness scores (generated multiple times a day) and associated features. | `/metrics-service/metrics/trainingreadiness/{date}` | [12345678_TRAINING_READINESS_2025-08-07T12:00:00Z.json](./example_data/12345678_TRAINING_READINESS_2025-08-07T12:00:00Z.json) |
| **Training Status** | VO2 max (generic and cycling) including heat and altitude acclimation, training load balance (low and high aerobic, anaerobic) with targets, acute/chronic workload ratio (ACWR), and feedback. | `/metrics-service/metrics/trainingstatus/aggregated/{date}` | [12345678_TRAINING_STATUS_2025-08-07T12:00:00Z.json](./example_data/12345678_TRAINING_STATUS_2025-08-07T12:00:00Z.json) |
| **Steps** | Number of steps and activity level (sedentary, sleeping, active, etc.) (15 mins interval time-series). | `/wellness-service/wellness/dailySummaryChart/{display_name}?date={date}` | [12345678_STEPS_2025-08-07T12:00:00Z.json](./example_data/12345678_STEPS_2025-08-07T12:00:00Z.json) |
| **Floors** | Floors climbed and descended (15 mins interval time-series). | `/wellness-service/wellness/floorsChartData/daily/{date}` | [12345678_FLOORS_2025-08-07T12:00:00Z.json](./example_data/12345678_FLOORS_2025-08-07T12:00:00Z.json) |
| **Intensity Minutes** | Weekly and daily moderate/vigorous intensity minutes with time-series data and goal tracking. | `/wellness-service/wellness/daily/im/{date}` | [12345678_INTENSITY_MINUTES_2025-08-07T12:00:00Z.json](./example_data/12345678_INTENSITY_MINUTES_2025-08-07T12:00:00Z.json) |
| **Body Composition** | Scale weigh-ins: weight, BMI, body fat %, body water %, bone mass, muscle mass, physique rating, visceral fat, metabolic age. Every weigh-in is captured when the user weighs more than once a day. | `/weight-service/weight/range/{start}/{end}?includeAll=true` | [12345678_BODY_COMPOSITION_2025-08-07T12:00:00Z.json](./example_data/12345678_BODY_COMPOSITION_2025-08-07T12:00:00Z.json) |
| **Personal Records** | All-time personal bests steps, running, cycling, swimming, strength. | `/personalrecord-service/personalrecord/prs/{display_name}` | [12345678_PERSONAL_RECORDS_2025-08-07T12:00:00Z.json](./example_data/12345678_PERSONAL_RECORDS_2025-08-07T12:00:00Z.json) |
...
| **Running Tolerance** | Daily biomechanical running-load model: total impact load, running distance, tolerated running-load ceiling, and week grouping. Populated only for accounts with a compatible watch. | `/metrics-service/metrics/runningtolerance/stats?startDate={start}&endDate={end}&aggregation=daily` | [12345678_RUNNING_TOLERANCE_2025-08-07T12:00:00Z.json](./example_data/12345678_RUNNING_TOLERANCE_2025-08-07T12:00:00Z.json) |
| **User Profile** | User profile settings including gender, weight, height, birthday, VO2 max (running and cycling), and lactate threshold (speed and heart rate). | `/userprofile-service/userprofile/user-settings` | [12345678_USER_PROFILE_2025-08-07T12:00:00Z.json](./example_data/12345678_USER_PROFILE_2025-08-07T12:00:00Z.json) |
...
| **Multi-Sport Legs** | Per-leg child activities of a multi-sport (duathlon/triathlon) parent, fetched via the parent's `metadataDTO.childIds`. Each leg is stored as its own activity linked by `parent_activity_id`, with per-leg sport aggregates from the leg's `summaryDTO`. | `/activity-service/activity/{activity_id}` | [12345678_MULTISPORT_CHILDREN_20000000001_2025-08-07T12:00:00Z.json](./example_data/12345678_MULTISPORT_CHILDREN_20000000001_2025-08-07T12:00:00Z.json) |
| **Exercise Sets** | Per-set granular data from strength training activities, including exercise categories, repetition counts, weight, and set-level timing. Per-exercise aggregates are derived from summarized exercise data in the activities list. | `/activity-service/activity/{activity_id}/exerciseSets` | [12345678_EXERCISE_SETS_19983039914_2025-08-07T12:00:00Z.json](./example_data/12345678_EXERCISE_SETS_19983039914_2025-08-07T12:00:00Z.json) |
...
The [garmin-health-data](https://pypi.org/project/garmin-health-data/) package provides a standalone Python CLI tool for extracting Garmin Connect health and fitness data to a local SQLite database. For minimal setup requirements, the database schema, extraction logic, and data processing code mirror this OpenETL pipeline, adapted to run without the enterprise-grade orchestration (Airflow) and time-series optimization (TimescaleDB) that OpenETL provides. This package is ideal for users who want a quick and easy way to extract their Garmin data locally in a well-structured SQLite file.
...
* **Target tables**: `garmin.activity_ts_metric` (hypertable), `activity_lap_metric`, `activity_split_metric`, `activity_hrv`, `swim_length`, `activity_event`.
* **Database method**: Delete+insert strategy for idempotent reprocessing. Deletes all existing rows for the activity across the FIT-derived child tables (`activity_ts_metric`, `activity_lap_metric`, `activity_split_metric`, `activity_path`, `activity_hrv`, `swim_length`, `activity_event`), then inserts fresh data via `session.add_all()`. This approach handles added/removed laps, splits, lengths, events, or records between reprocesses without UNIQUE constraint violations. FIT `session`-message metrics instead upsert into `supplemental_activity_metric` (keyed by `(activity_id, metric)`), since those metric names never collide with the API-sourced rows. After insertion, sets `activity.ts_data_available` to reflect whether time-series records were found in the FIT file.
* **Data processing**: Uses [`fitdecode`](https://pypi.org/project/fitdecode/) library for binary FIT file parsing with frame-based processing. Processes record, lap, split, and hrv frame types: Record frames (time-series sensor data with two-pass timestamp processing → `activity_ts_metric`), Lap frames (device-triggered segments with metric extraction → `activity_lap_metric`), Split frames (algorithmic intervals with type classification: rwd_run, rwd_walk, rwd_stand, interval_active → `activity_split_metric`), hrv frames (beat-to-beat R-R intervals in seconds, flattened across all hrv frames into a single ordered array → `activity_hrv`), length frames (per-length pool swim data, active and idle, extracted by name into typed rows → `swim_length`), event frames (every FIT event message: common event/event_type/timestamp fields as columns plus remaining fields as a JSON object → `activity_event`), and session frames (an allowlist of FIT-only fields the Connect API never exposes — advanced cycling pedal dynamics, seated/standing power and cadence, mechanical work, subjective effort — upserted into `supplemental_activity_metric`). Handles array fields using suffix indexing (`_1`, `_2`, etc.) for multiple values per timestamp with field name validation (`field.name is not None`), "unknown" field filtering, and null value checking. Includes binary format validation, type conversion error handling (ValueError, TypeError), and graceful degradation for corrupt data.
