"""
SQLAlchemy models to facilitate Garmin Connect data processing and database interaction.

These models reflect database tables for storing Garmin Connect wellness and activity
data defined in the tables.ddl file.
"""
...
# Set update_ts for models that allow record updates.
InsertBase = make_base(schema="garmin", metadata=_shared_metadata)
UpsertBase = make_base(
    schema="garmin", include_update_ts=True, metadata=_shared_metadata
)


class User(InsertBase):
    """
...
class Activity(UpsertBase):
    """
    Main activity table containing core aggregate metrics common across most activity.

    types. Additional aggregate metrics may be available in separate tables:
    ...
    has_polyline = Column(Boolean, nullable=False)
    has_images = Column(Boolean, nullable=False)
    ...
    purposeful = Column(Boolean, nullable=False)
    favorite = Column(Boolean, nullable=False)
    elevation_corrected = Column(Boolean)
    atp_activity = Column(Boolean)
...
class CyclingAggMetrics(UpsertBase):
    """
...
class RunningAggMetrics(UpsertBase):
    """
...
class SupplementalActivityMetric(UpsertBase):
    """
...
class SleepRestlessMoment(InsertBase):
    """
...
class BreathingDisruption(InsertBase):
    """
...
class TrainingReadiness(UpsertBase):
    """
...
class IntensityMinutes(InsertBase):
    """
...
class PersonalRecord(InsertBase):
    ...
    Each record represents a best performance for a specific type and user. The latest
    column indicates the most recent personal record for each user and type.
    ...
    user_id = Column(BigInteger, fkey("garmin", "user", "user_id"), primary_key=True)
    activity_id = Column(BigInteger, fkey("garmin", "activity", "activity_id"))
    timestamp = Column(DateTime(timezone=True), primary_key=True)
...
class RacePredictions(InsertBase):
    """
...
class ActivitySplitMetric(InsertBase):
    """
...
class ActivityLapMetric(InsertBase):
    """
